...
public class BookDaoImpl implements BookDao {
	...
	public void insert(Book book) throws BookException {
		try {
			...
			template.update(sql.toString(), 
					book.getTitle(), book.getAuthor(), book.getPublisher(), book.getPrice(), book.getDescription());
		} catch (Exception e) {
	...
	public Book select(String isbn) throws BookException {
		...
		try {
			...
			book = template.queryForObject(sql, new BeanPropertyRowMapper<Book>(Book.class));
		} catch (Exception e) {
			throw new BookException(e.getMessage());
		}
		return book;
