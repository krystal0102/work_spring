	...
	public void sendEmail(Users user) {
		MimeMessagePreparator preparator = new MimeMessagePreparator() {
			...
			public void prepare(MimeMessage mimeMessage) throws Exception {
				...
				helper.setFrom("lsaj29@gmail.com");		 			// 보내는 사람 이메일 주소
				helper.setTo(user.getEmail());			 			// 받는 사람 이메일 주소
				helper.setSubject("[KOITT] SpringBatchEx03");		// 메일 제목
				helper.setText(text, true);							// 메일 내용, true는 HTML mail
				
				
				// 받는 사람 이메일 값과 메일 내용 테스트위해 콘솔에 출력
				System.out.println(user.getEmail());
				System.out.println(text);
				
				/*
				 * 내장 리소스 
				 * (FreeMarker 템플릿(HTML)에서 cid:image.jpg 로 사용하면 
				 * img/lights.jpg 경로에 해당하는 이미지를 사용하는 것)
				 */
					
				// 첨부파일
				helper.addAttachment("document.csv", new FileSystemResource("C:/sample/prac03/board.csv"));
			}
