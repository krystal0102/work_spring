...
// 스프링 테스트 컨텍스트 프레임 워크의 JUnit확장기능 지정 
@RunWith(SpringJUnit4ClassRunner.class)

// 테스트 컨텍스트가 자동으로 만들어줄 어플리케이션 컨텍스트의 위치 지정
...
public class UserDaoTest {
	...
	// @Test가 실행되기 전에 먼저 실행되어야 하는 메소드를 정의한다
	@Before 
	public void setUp() {
	 ...
	 * 모든 테스트는 실행 순서에 관계 없이 독립적으로 항상 동일한 결과를 낼 수 있도록 한다.
	 */
	 ...
	 * 									존재하지 않을 경우에			assertThat(dao.getCount(), is(0))
	 * 
	 * 행위		무엇을 할 때			존재하지 않는 id로 get()		dao.get("unknown_id");
	 * 결과		어떤 결과가 나온다		실행하면 특별한 				@Test(expected=EmptyResultDataAccessException.class)
	 * 									예외가 던져진다	
	 */

	// 위와 같이 작성하면 기능 설계, 코드 구현, 테스트 세 작업이 동시에 끝!!!!!!!
	...
	/*
	 * JUnit이 하나의 테스트 클래스를 가져와 테스트를 수행하는 방식 (7단계0
	 * 
	 * 1. 테스트 클래스에서 @Test가 붙은 public이고 void형이며 파라미터가 없는 테스트 메소드를 모두 찾는다
	 * 
	 * 2. 테스트 클래스의 객체를 하나 만든다. (객체를 매번 새로 만드는 이유는 독립적으로 실행됨을 확실히 보장해주기 위해)
	 * 
