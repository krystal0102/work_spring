...
# 4. board 테이블과 users테이블 EQUI JOIN
...
SELECT b.no, b.title, b.content, b.user_no, b.regdate, b.attachment, 
	   	u.email, u.name, u.attachment as "uattachment"
		FROM board b, users u
		WHERE b.user_no = u.no  AND b.no = 4 ORDER BY b.no DESC;
		
# 6. 사용자 번호 3번 유저와 같이 관리자와 일반 사용자 권한을 함께 입력할 경우 (오라클 사용시)
INSERT ALL
...
# 6-1. 6번을 My SQL로 변경
INSERT INTO users_authority(users_no, authority_id) VALUESE (8,10), (8,20);
