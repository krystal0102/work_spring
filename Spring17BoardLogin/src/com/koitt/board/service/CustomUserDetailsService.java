...
@Service	//("customUserDatailsService")
public class CustomUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
		...
		try {
			...
			/*
			 *  해당 사용자가 존재하지 않으면 
			 *  이미 정의된 UsernameNotFoundException을 이용하여 예외를 생성해서 던져주면
			 *  스프링이 예외처리를 히게 된다.
			 */
			if (users == null) {
	          ...
	          * authorities
	          */
			return new User(users.getEmail(), users.getPassword(), true, true, true, true, this.getGranteAuthorities(users));
	...
	// 사용자에게 권한을 부여하기 위해 권한 목록을 리턴하는 메소드
	private List<GrantedAuthority> getGranteAuthorities(Users users) {
		
		// 최종적으로 스프링에게 전달할 리스트 (타입은 GrantedAuthority)
		List<GrantedAuthority> auths = new ArrayList<>();
		
		/*
		 * 데이터베이스에 저장되어 있던 해당 사용자의 권한을 하나씩 끄집어내어 GrantedAuthority 리스트에 추가해주는 작업
		 * 스프링 3버전에서는 "ROLE_"이라는 문자열을 추가 안해도 되지만
		 * 4버전 이후부터는 "ROLE_"이라는 문자열을 권한이름 앞에 붙여줘야 정상 동작
		 */
