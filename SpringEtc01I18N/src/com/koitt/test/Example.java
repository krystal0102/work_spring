...
public class Example {
	 ...
	 * 						new Object[]{"Car", "Computer"}를 전달하면 {0} 대신에 Car, {1} 대신에 Computer로 대치
	 * 세 번째 파라미터: 첫 번째 파라미터에 해당하는 code(변수명)이 없을 경우 기본으로 출력하는 문자열
	 * 네 번째 파라미터: 어떤 언어로 문자열을 가져올 것인지 설정
	 * 					ex> Locale.KOREAN: greeting_ki.properties 의 문자열을 가져옴
